...
g.start}b=b.previousSibling}l.data=n.reverse();return l})})(Popcorn);(function(o){o.parser("parseSBV",function(b){var l={title:"",remote:"",data:[]},m=[],f=0,k=0,n=function(a){a=a.split(":");var h=a.length-1,i;try{i=parseInt(a[h-1],10)*60+parseFloat(a[h],10);if(h===2)i+=parseInt(a[0],10)*3600}catch(p){throw"Bad cue";}return i},g=function(a,h){var i={};i[a]=h;return i};b=b.text.split(/(?:\r\n|\r|\n)/gm);for(k=b.length;f<k;){var j={},e=[],c=b[f++].split(",");try{j.start=n(c[0]);for(j.end=n(c[1]);f<k&&b[f];)e.push(b[f++]);j.text=e.join("<br />");m.push(g("subtitle",j))}catch(d){for(;f<
