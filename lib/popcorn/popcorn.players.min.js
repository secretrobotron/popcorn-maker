...
f.extend({},c,{end:o[q],clipIdx:q})}i=Object.keys(d).map(function(t){return+t});c=n(i[0],i[1]);for(i=0;i<c.length;i++){l={};q=c[i];var s=d[q];if(s){r=this.inOuts.ofVideos[q];o=s.clipIdx;r=n(r["in"],r.out);if(s.start){l.start=r[o];l.end=r[r.length-1]}if(s.end){l.start=r[0];l.end=r[o]}}else{l.start=this.inOuts.ofVideos[q]["in"];l.end=this.inOuts.ofVideos[q].out}this.playlist[q][b](f.extend({},a,l))}return this}})})(this,Popcorn);var onYouTubePlayerReady=function(j){onYouTubePlayerReady[j]&&onYouTubePlayerReady[j]()};onYouTubePlayerReady.stateChangeEventHandler={};
...
return this._container}},getStyle:function(h){var p=this._resource||this._container;return p.currentStyle?p.currentStyle[h]:j.getComputedStyle?f.defaultView.getComputedStyle(p,null).getPropertyValue(h):p.style[h]}}})(window,document);(function(j,f){function h(b,a){if(b.currentStyle)return b.currentStyle[a];else if(f.getComputedStyle)return document.defaultView.getComputedStyle(b,null).getPropertyValue(a)}function p(b){return'<div><a href="'+b.user.profile+'"><img width="16px height="16px" src="'+b.user.avatar+'"></img>'+b.user.name+"</a> at "+function(a){var d=n(a/60);a=k(a%60);return d+"."+(a<10?"0":"")+a}(b.start)+" "+function(a){function d(c,i){return c+" "+i+(c>1?"s":"")+" ago"}a=((new Date).getTime()-a.getTime())/1E3;if(a<
